...
C          SMRLIB.LST for any of the WIMS-D libraries (the
C          q-value is not strongly dependent on WIMS-library).
...
C
C 1. Run BNCHALL.BAT for all WIMS-libraries wished (do not delete wims
C    output files for one lib to be used as reference for q-value
C    calculations)
...
C
      DO I1=1,NBEN
C
C    >READ BENCHMARK NAME AND NUMBER OF OUTPUTS FOR BENCH.,I1 FROM
C     QVALUE.INP
C
       READ(1,4)NOMBEN
       READ(1,2)NOUT
...
C
       DO J1=1,NOUT
C
C    >READ WIMS-OUTP NAME AND NUMB.OF CASES FOR BENH I1, OUT J1 FROM
C     QVALUE.INP
C
        READ(1,5)NOMOUT
        READ(1,2)NCAS
...
C
        DO K1=1,NCAS
C
C    >READ Keff VALUES FOR BENCH I1,OUT J1,CASE K1 FOR ALL LIBS
C     FROM SMRLIB.LST
C
  70     read(2,11)LINE
