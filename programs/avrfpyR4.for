...
C-M          It is given by:
C-M            EPU = (1.E24/(24*3600)) * ALOG(2) * EPS / FLX
...
C-M            EPD = (1.E24/(24*3600)) * ALOG(2) / (EPS*FLUX)
...
C* Define the weighting function for the F.P yields
   ...
   37 CONTINUE
      FRR( 4)=0.
      FRR( 2)=(1./EGB(2))*FIN(2)/ALOG(EGB(3)/EGB(2))
...
C* If cross sections file is given, find the fissile parent fiss.x-sect.
      ...
      ELSE
        ...
        FRR(2)=FRR(2) * FTH * (2./SQRT(3.1415926)) * SQRT(0.0253/EGB(2))
        IF(FTH.LE.0)
     1  FRR(3)=FAV(3)/(EGB(4)-EGB(3))
      END IF
...
C-Purpose: Assign decay modes to the selected nuclide
      CHARACTER*66   REC
      DIMENSION      BRR(1), MDD(1), IZAP(1), IZOP(1)
...
C-
      CHARACTER*120 REC
      CHARACTER*11  RCT
      DIMENSION     ZAP(1),XTH(1),RIN(1),XFS(1)
C*
...
C*   SF - total spectrum
      ...
      B =ALOG(FRR(IG+1)/FRR(IG))/ALOG(EGB(IG+1)/EGB(IG))
      B1=B+1.
      IF(ABS(B1).LT.1.E-4) THEN
        SG=SG+FRR(IG)*EA*ALOG(EB/EA)
      ELSE
...
C* Print warning when yield at E2 for the current isotope is missing
   61 IF(LER.GT. 0) WRITE(LER,801) 2,IFIX(ZAP1(I)+0.1),E2
...
C* Print warning when yield at E1 for the current isotope is missing
   63 IF(LER.GT. 0) WRITE(LER,801) 1,IFIX(ZAP2(L)+0.1),E1
      ZP=ZAP2(L)
...
C     B =ALOG(FRR(IG+1)/FRR(IG))/ALOG(EGB(IG+1)/EGB(IG))
C     B1=B+1.
C     B2=B+2.
C     A =FRR(IG)/EGB(IG)**B
C     G =(Y2-Y1)/(E2-E1)
...
C       Y = Y + C*A*ALOG(EB/EA)
C     ELSE IF(ABS(B2).LT.1.E-4) THEN
C       Y = Y + G*A*ALOG(EB/EA) - C*A*(1./EB - 1./EA)
C     ELSE
