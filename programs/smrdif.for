...
C-V 01/09- Print banner, assign variables to logical units (A.Trkov)
C-M
C-M  Manual for Program SMRDIF
C-M  =========================
C-M  The program reads the K-eff and four spectral indices from a
C-M  reference file and from the WEDOG2 output, which summarizes the
C-M  results of a WIMS-D calculation.
C-M
...
C-
     ...
     1              ,ND(5),RV(5),AV(5),AE(5)
      ...
      DATA BLN/'          '/
      DATA LRE,LIN,LOU,LKB,LTT/ 1, 2, 8, 5, 6 /
C*
      WRITE(LTT,91)
      WRITE(LTT,91) ' SMRDIF - Compare lattice spectr.indices'
      WRITE(LTT,91) ' ======================================='
      WRITE(LTT,91)
C* Define the filenames
      WRITE(LTT,91) '$Enter the reference filename         : '
      READ (LKB,91) FLNM
      FLNR=FLNM
      OPEN (UNIT=LRE,FILE=FLNM,STATUS='OLD')
      WRITE(LTT,91) '$Enter the compared  filename         : '
      READ (LKB,91) FLNM
      FLNC=FLNM
      OPEN (UNIT=2,FILE=FLNM,STATUS='OLD')
      WRITE(LTT,91) '$Enter the output filename            : '
      READ (LKB,91) FLNM
      OPEN (UNIT=8,FILE=FLNM,STATUS='UNKNOWN',CARRIAGECONTROL='LIST')
      WRITE(LTT,91) '$Enter non-blank to print % diff.     : '
      READ (LKB,91) FLNM
C* Write the header to output
      WRITE(LOU,91)
      WRITE(LOU,91) ' SMRDIF - Compare lattice spectr.indices'
      WRITE(LOU,91) ' ======================================='
      WRITE(LOU,91)
      WRITE(LOU,91) ' Reference file                       : ',FLNR
      WRITE(LOU,91) ' Compared  file                       : ',FLNC
      WRITE(LOU,91)
      WRITE(LOU,91) '   LATTICE      K-eff       Rho28       '
     1           ,'  Del25          Del28         ConvR    '
      WRITE(LOU,91) '  ======================================'
     1           ,'================================        '
      NE=0
C* Read a line from reference and compared data base
   20 READ (LRE,92,END=80,ERR=80) LBL,PR(1),IKR,(PR(J),ER(J),J=2,5)
      READ (LIN,94,END=80,ERR=80)     PC(1),    (PC(J)      ,J=2,5)
      NE=NE+1
...
C* Case: Print absolute differences
      IF(FLNM.NE.'                                        ') GO TO 30
      WRITE(LOU,92) LBL,PR(1),IKR,(PR(J),ER(J   ),J=2,5)
      WRITE(LOU,92) BLN,PC(1),IKC,(PC(J),EC(J,NE),J=2,5)
      WRITE(LOU,92)
      GO TO 20
...
C* Write the relative differences for the current lattice
      WRITE(LOU,93) LBL,(PR(J),HR(J),J=1,5)
      WRITE(LOU,93) BLN,(PC(J),HC(J),J=1,5)
      WRITE(LOU,93)
      GO TO 20
C* Input exhausted - print average if rel.differences requested
   ...
   86 CONTINUE
      WRITE(LOU,98) '   Average',(RV(J)      ,J=1,5),(AV(J),AE(J),J=1,5)
   90 STOP 'SMRDIF Completed'
   91 FORMAT(2A40)
